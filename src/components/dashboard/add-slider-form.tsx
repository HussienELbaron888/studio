...
import { saveSlide, SlideValues } from "@/utils/saveSlide";
import Image from "next/image";
import { cn } from "@/lib/utils";

type AddSliderFormProps = {
  setDialogOpen: (open: boolean) => void;
};

export function AddSliderForm({ setDialogOpen }: AddSliderFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveSlide(values, imageFile);
      toast({ title: "Success", description: "Slide added successfully!" });
      setDialogOpen(false);
    } catch (error: any) {
      console.error("Submission failed:", error);
      toast({ title: "Error", description: `Failed to add slide: ${error.message}`, variant: "destructive" });
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="description-ar">Description (العربية)</Label>
        <Textarea id="description-ar" value={descriptionAr} onChange={e => setDescriptionAr(e.target.value)} required />
      </div>
      <div className="space-y-2">
        <Label htmlFor="description-en">Description (English)</Label>
        <Textarea id="description-en" value={descriptionEn} onChange={e => setDescriptionEn(e.target.value)} required />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="button-text-ar">Button Text (العربية)</Label>
          <Input id="button-text-ar" value={buttonTextAr} onChange={e => setButtonTextAr(e.target.value)} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="button-text-en">Button Text (English)</Label>
          <Input id="button-text-en" value={buttonTextEn} onChange={e => setButtonTextEn(e.target.value)} required />
        </div>
      </div>
      <div className="space-y-2">
          <Label htmlFor="button-href">Button Link</Label>
          <Input id="button-href" value={buttonHref} onChange={e => setButtonHref(e.target.value)} placeholder="/events" required />
      </div>
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        {previewUrl ? (
          ...
          >
            ...
            <Input ref={fileInputRef} id="dropzone-file" type="file" className="hidden" accept="image/*" onChange={handleFileChange} required />
          </div>
        )}
      </div>

      <Button type="submit" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving...</> : "Save Slide"}
      </Button>
    </form>
  );
}
