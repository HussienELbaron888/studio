...
import { saveEvent, EventValues } from "@/utils/saveEvent";
...
export function AddEventForm({ setDialogOpen }: AddEventFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveEvent(values, imageFile);
      toast({
        title: "Success",
    ...
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="title-ar">{content.eventTitleLabel} (العربية)</Label>
          <Input id="title-ar" value={titleAr} onChange={e => setTitleAr(e.target.value)} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="title-en">Event Title (English)</Label>
          <Input id="title-en" value={titleEn} onChange={e => setTitleEn(e.target.value)} required />
        </div>
      </div>
      <div className="space-y-2">
        <Label htmlFor="description-ar">{content.eventDescriptionLabel} (العربية)</Label>
      ...
      <div className="space-y-2">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="location-ar">{content.eventLocationLabel} (العربية)</Label>
          <Input id="location-ar" value={locationAr} onChange={e => setLocationAr(e.target.value)} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="location-en">Location (English)</Label>
          <Input id="location-en" value={locationEn} onChange={e => setLocationEn(e.target.value)} required />
        </div>
      </div>
      <div className="space-y-2">
        <Label htmlFor="price">{content.priceLabel}</Label>
        <Input id="price" type="number" placeholder="اختياري" value={price} onChange={e => setPrice(e.target.value === '' ? '' : Number(e.target.value))} />
      </div>
