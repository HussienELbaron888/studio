...
export function EditTripForm({ trip, setDialogOpen }: EditTripFormProps) {
  ...
  useEffect(() => {
    ...
    if (trip) {
      ...
      setDestinationAr(trip.destination.ar);
      setDestinationEn(trip.destination.en);
      setScheduleAr(trip.schedule.ar);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateTrip(trip.id, values, imageFile, trip.image_path);
      toast({
        title: "Success",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="title-en-edit">Trip Title (English)</Label>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="destination-en-edit">Destination (English)</Label>
          <Input id="destination-en-edit" value={destinationEn} onChange={e => setDestinationEn(e.target.value)} required />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="schedule-en-edit" placeholder="e.g., Every Saturday" value={scheduleEn} onChange={e => setScheduleEn(e.target.value)} />
        </div>
      </div>
      <div className="space-y-2">
